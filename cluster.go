...
func (c *clusterClient) _refresh() (err error) {
	...
	for i := 0; i < cap(results); i++ {
		...
		if result.reply.val.GetValuesLen() != 0 {
			break
		}
	}
	if err != nil {
...
func parseSlots(slots RedisMessage, defaultAddr string) map[string]group {
	groups := make(map[string]group, slots.GetValuesLen())
	for _, v := range slots.GetValuesVal() {
		master := parseEndpoint(defaultAddr, v.GetValuesVal()[2].GetValuesVal()[0].GetStringVal(), v.GetValuesVal()[2].GetValuesVal()[1].integer)
		if master == "" {
			continue
		}
		g, ok := groups[master]
		if !ok {
			...
			g.nodes = make(nodes, 0, v.GetValuesLen()-2)
			for i := 2; i < v.GetValuesLen(); i++ {
				if dst := parseEndpoint(defaultAddr, v.GetValuesVal()[i].GetValuesVal()[0].GetStringVal(), v.GetValuesVal()[i].GetValuesVal()[1].integer); dst != "" {
					g.nodes = append(g.nodes, ReplicaInfo{Addr: dst})
				}
			}
		}
		g.slots = append(g.slots, [2]int64{v.GetValuesVal()[0].integer, v.GetValuesVal()[1].integer})
		groups[master] = g
	}
	return groups
}
...
func parseShards(shards RedisMessage, defaultAddr string, tls bool) map[string]group {
	groups := make(map[string]group, shards.GetValuesLen())
	for _, v := range shards.GetValuesVal() {
		m := -1
		shard, _ := v.AsMap()
		slots := RedisMessageGetValuesVal(shard["slots"].values, shard["slots"].integer)
		_nodes := RedisMessageGetValuesVal(shard["nodes"].values, shard["nodes"].integer)
		g := group{
		...
		for _, n := range _nodes {
			dict, _ := n.AsMap()
			if RedisMessageGetStringVal(dict["health"].string, dict["health"].integer) != "online" {
				continue
			}
			...
			if dst := parseEndpoint(defaultAddr, RedisMessageGetStringVal(dict["endpoint"].string, dict["endpoint"].integer), port); dst != "" {
				if RedisMessageGetStringVal(dict["role"].string, dict["role"].integer) == "master" {
					m = len(g.nodes)
				}
				g.nodes = append(g.nodes, ReplicaInfo{Addr: dst})
			}
	...
	for i, resp := range resps {
		...
		if mode != RedirectNone {
			...
			if hasInit && ei < i { // find out if there is a transaction block or not.
				...
				if mi >= 0 && ei < len(commands) && isMulti(commands[mi]) && isExec(commands[ei]) && RedisMessageGetStringVal(resps[mi].val.string, resps[mi].val.integer) == ok { // a transaction is found.
					mu.Lock()
					retries.Redirects++
					nr := retries.m[nc]
					if nr == nil {
