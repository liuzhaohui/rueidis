	...
	t.Run("single client", func(t *testing.T) {
		...
		t.Run("Delegate DisabledCache MGetCache", func(t *testing.T) {
			m.DoFn = func(cmd Completed) RedisResult {
				...
				return newResult(NewRedisMessage('*',  []RedisMessage{NewRedisMessage('+', "1"), NewRedisMessage('+', "2")}), nil)
			}
			...
			m.DoMultiCacheFn = func(multi ...CacheableTTL) *redisresults {
				if reflect.DeepEqual(multi[0].Cmd.Commands(), []string{"GET", "1"}) && multi[0].TTL == 100 &&
					...
					return &redisresults{s: []RedisResult{
						...
						newResult(NewRedisMessage('+', "2"), nil),
					}}
				}
				t.Fatalf("unexpected command %v", multi)
				return nil
			}
			if v, err := MGetCache(client, context.Background(), 100, []string{"1", "2"}); err != nil || RedisMessageGetStringVal(v["1"].string, v["1"].integer) != "1" || RedisMessageGetStringVal(v["2"].string, v["2"].integer) != "2" {
				t.Fatalf("unexpected response %v %v", v, err)
			}
		})
		...
		t.Run("Delegate DisabledCache DoCache", func(t *testing.T) {
			...
			m.DoMultiFn = func(cmd ...Completed) *redisresults {
				...
				for i, key := range keys {
					...
					result[i] = newResult(NewRedisMessage('+', key), nil)
				}
			...
			for _, key := range keys {
				if RedisMessageGetStringVal(v[key].string, v[key].integer) != key {
...
func TestMGet(t *testing.T) {
	...
	t.Run("single client", func(t *testing.T) {
		...
		t.Run("Delegate Do", func(t *testing.T) {
			...
			if v, err := MGet(client, context.Background(), []string{"1", "2"}); err != nil || RedisMessageGetStringVal(v["1"].string, v["1"].integer) != "1" || RedisMessageGetStringVal(v["2"].string, v["2"].integer) != "2" {
				t.Fatalf("unexpected response %v %v", v, err)
			}
		})
		...
		t.Run("Delegate Do", func(t *testing.T) {
			...
			m.DoMultiFn = func(cmd ...Completed) *redisresults {
				...
				for i, c := range cmd {
					...
					result[i] = newResult(NewRedisMessage('+', "OK"), nil)
				}
				if len(cpy) != 0 {
					t.Fatalf("unexpected command %v", cmd)
					return nil
...
func TestJsonMGetCache(t *testing.T) {
	...
	t.Run("single client", func(t *testing.T) {
		...
		t.Run("Delegate DoCache", func(t *testing.T) {
			...
			if v, err := JsonMGetCache(client, context.Background(), 100, []string{"1", "2"}, "$"); err != nil || RedisMessageGetStringVal(v["1"].string, v["1"].integer) != "1" || RedisMessageGetStringVal(v["2"].string, v["2"].integer) != "2" {
				t.Fatalf("unexpected response %v %v", v, err)
			}
		})
		t.Run("Delegate DoCache Empty", func(t *testing.T) {
...
func TestJsonMGet(t *testing.T) {
	...
	t.Run("single client", func(t *testing.T) {
		...
		t.Run("Delegate Do", func(t *testing.T) {
			...
			if v, err := JsonMGet(client, context.Background(), []string{"1", "2"}, "$"); err != nil || RedisMessageGetStringVal(v["1"].string, v["1"].integer) != "1" || RedisMessageGetStringVal(v["2"].string, v["2"].integer) != "2" {
				t.Fatalf("unexpected response %v %v", v, err)
			}
		})
		t.Run("Delegate Do Empty", func(t *testing.T) {
...
func TestDecodeSliceOfJSON(t *testing.T) {
	...
	values := []RedisMessage{
		NewRedisMessage('+', `{"ID":1, "Name": "n1", "Inners": [{"Field": "f1"}]}`),
		NewRedisMessage('+', `{"ID":2, "Name": "n2", "Inners": [{"Field": "f2"}]}`),
	}
	result := RedisResult{val: NewRedisMessage('*',  values)}
	...
	t.Run("Scan []*T: has nil error message", func(t *testing.T) {
		...
		hasNilResult := RedisResult{val: NewRedisMessage('*',  hasNilValues)}
	...
	t.Run("has non-nil error message in result", func(t *testing.T) {
		hasErrValues := []RedisMessage{
			NewRedisMessage('+', `{"ID":1, "Name": "n1", "Inners": [{"Field": "f1"}]}`),
			NewRedisMessage('-', `invalid`),
		}
		hasErrResult := RedisResult{val: NewRedisMessage('*',  hasErrValues)}
