...
func TestLRU(t *testing.T) {
	...
	t.Run("Cache Evict", func(t *testing.T) {
		...
		for i := 1; i <= Entries; i++ {
			...
			m := NewRedisMessage('+', strconv.Itoa(i))
			m.setExpireAt(time.Now().Add(PTTL * time.Millisecond).UnixMilli())
		...
		} else if v.GetStringVal() != strconv.Itoa(Entries) {
	...
	t.Run("Cache Delete", func(t *testing.T) {
		lru := setup(t)
		lru.Delete([]RedisMessage{NewRedisMessage('+', "0")})
		if v, _ := lru.Flight("0", "GET", TTL, time.Now()); v.typ != 0 {
	...
	t.Run("Cache Flush", func(t *testing.T) {
		...
		for i := 1; i < Entries; i++ {
			if v, _ := lru.Flight(strconv.Itoa(i), "GET", TTL, time.Now()); v.GetStringVal() != strconv.Itoa(i) {
	...
	t.Run("Cache Close", func(t *testing.T) {
		...
		m := NewRedisMessage('+', "this Update should have no effect")
		m.setExpireAt(time.Now().Add(PTTL * time.Millisecond).UnixMilli())
		lru.Update("1", "GET", m)
	...
	t.Run("Batch Cache Flush", func(t *testing.T) {
		...
		for i := 1; i < Entries; i++ {
			if v, _ := flights(lru, time.Now(), TTL, "GET", strconv.Itoa(i)); v.GetStringVal() != strconv.Itoa(i) {
				t.Fatalf("got unexpected value before flush all: %v", v)
			}
		}
		lru.Delete(nil)
